...
#
set.seed(123)
excluded_variables <- sample(1:nrow(insurance_data), 30)
excluded_data <- insurance_data[excluded_variables,]
included_data <- insurance_data[-excluded_variables,]

summary_stats <- sapply(included_data[c("Charges", "Age", "BMI", "Children")], function(x) {
c(min=min(x),
  max=max(x),
  mean=mean(x),
  median=median(x),
...
```{r}
#
cor(included_data[c("Charges", "Age", "BMI", "Children")])
```
...
insurance$LogCharges <- log(insurance$Charges + 1)

transformed_model <- lm(LogCharges ~ Age + BMI + Children + Female + Smoker + WinterSprings + WinterPark + Oviedo, data = insurance)
